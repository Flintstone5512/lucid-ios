...
export default function SessionScreen() {
  ...
  const [startTime, setStartTime] = useState<number>(Date.now()); // 🔥 NEW

  const { selectedDeckId, setStatePatch, streak, usage } =
    useRefocusStore();
  const { plan, adMode } = useRefocusStore();
  ...
  useEffect(() => {
    setStartTime(Date.now()); // 🔥 RESET TIMER PER CARD
  }, [index]);

  if (!selectedDeckId) {
    return (
      <View style={{ flex: 1, justifyContent: "center", padding: 24, backgroundColor: "#0e1424" }}>
  ...
  try {
    ...
    if (showAd) {
      showRewardedAd(async () => {
        try {
          /* 🔥 CRITICAL: GRANT REWARD AFTER AD */
          const rewardRes = await api.post("/unlock/reward", {
            type: "time", // 🔥 matches your backend
          });

          const unlock = rewardRes?.data?.unlock;

          /* 🔥 OPTIONAL: SYNC STATE (better UX) */
          if (unlock?.unlockedUntil) {
            const unlockUntil = new Date(unlock.unlockedUntil).getTime();

            // if you're using your store:
            try {
              const { setStatePatch } = useRefocusStore.getState();

              setStatePatch({
                unlockedUntil: unlockUntil,
              });
            } catch (err) {
              console.log("State sync skipped");
            }
          }
  ...
  async function answer(rating: string) {
    try {
      const responseTimeMs = Date.now() - startTime; // 🔥 NEW

      const payload = {
        cardId: card._id || card.id,
        deckId: card.deckId || selectedDeckId,
        rating,
        responseTimeMs, // 🔥 CRITICAL
      };
  ...
  if (completed) {
    ...
    return (
      ...
      >
        ...
        <Text style={{ color: "#D86732", fontSize: 28, fontWeight: "800", textAlign: "center", marginTop: 10 }}>
          You earned {minutes} minutes
        </Text>

        <Text style={{ color: "#A9BDDB", marginTop: 10, textAlign: "center" }}>
          Most people scroll.{"\n"}You progressed.
        </Text>

        <Pressable
          onPress={() => router.back()}
          style={{ marginTop: 30, backgroundColor: "#D86732", padding: 16, borderRadius: 14, width: "100%" }}
        >
    ...
    <View style={{ flex: 1, padding: 24, backgroundColor: "#0e1424" }}>
      ...
      {!showAnswer ? (
        <Pressable
          ...
          style={{ marginTop: 24, backgroundColor: "#1b2540", padding: 16, borderRadius: 14 }}
        >
