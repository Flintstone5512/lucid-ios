...
export async function showBlockingOverlay(packageName = "a blocked app") {
  if (isDev) {
    console.log("[DEV] showBlockingOverlay:", packageName);
    return { ok: true };
  }
...
export async function setBlockedApps(packages = []) {
  ...
  try {
    console.log("[NATIVE] raw setBlockedApps packages:", packages);
    ...
    console.log("[NATIVE] clean setBlockedApps packages:", cleanPackages);

    await EnforcementModule?.saveMonitoredPackages?.(cleanPackages);
    return { ok: true };
  } catch (err) {
...
export async function enableAndroidEnforcement(packages = []) {
  ...
  try {
    console.log("[NATIVE] raw enableAndroidEnforcement packages:", packages);
    ...
    console.log("[NATIVE] clean enableAndroidEnforcement packages:", cleanPackages);
