...
public class ScreenTimeModule: Module {
  ...
  // Hold a strong reference so the window stays alive until picker closes
  ...
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("presentAppPicker") { (promise: Promise) in
      ...
      // Load the previously saved selection so the picker opens pre-populated.
      // Without this, @State selection starts empty and Save writes empty tokens.
